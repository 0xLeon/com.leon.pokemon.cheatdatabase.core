...
<div id="main">
	<div class="cheatDatabase">
		...
		{else}
			{assign var='canEditEntry' value=$this->user->getPermission('mod.cheatDatabase.canEditEntry')}
			{assign var='canDeleteEntry' value=$this->user->getPermission('mod.cheatDatabase.canDeleteEntry')}
			...
			<div class="border borderMarginRemove">
				<table class="tableList">
					<thead>
						<tr class="tableHead">
							<th class="columnEntryID{if $sortField == 'entry.entryID'} active{/if}"{if $canEditEntry || $canDeleteEntry} colspan="2"{/if}>
								<div>
					...
					<tbody>
						...
						{foreach from=$entries item=$entry}
							<tr class="{cycle name='className'}" id="entryRow{@$entry->entryID}">
								{if $canEditEntry || $canDeleteEntry}
									<td class="columnIcon">
										{if $canEditEntry}<a href="index.php?form=CheatDatabaseEntryEdit&amp;entryID={$entry->entryID}{@SID_ARG_2ND}"><img src="{icon}editS.png{/icon}" alt="" title="{lang}wcf.cheatDatabase.entry.edit{/lang}" /></a>{/if}
										{if $canDeleteEntry}<a onclick="return confirm('{lang}wcf.cheatDatabase.entry.delete.sure{/lang}')" href="index.php?action=CheatDatabaseEntryDelete&amp;entryID={@$entry->entryID}&amp;t={@SECURITY_TOKEN}{@SID_ARG_2ND}"><img src="{icon}deleteS.png{/icon}" alt="" title="{lang}wcf.cheatDatabase.entry.delete{/lang}" /></a>{/if}
									</td>
								{/if}
